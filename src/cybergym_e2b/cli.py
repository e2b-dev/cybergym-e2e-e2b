...
def _run_options(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help="digest lock generated by `cybergym-e2b images lock`",
    )
    ...
    parser.add_argument(
        ...
        default="policy",
        help=(
            "policy: apply the network policy as written (default); restricted: allow only the "
            "policy's dependency and model hosts; permissive: no egress filter, for diagnostics"
        ),
    )
...
def _parser() -> argparse.ArgumentParser:
    ...
    image_lock = images_sub.add_parser(
        "lock", help="resolve mutable upstream images to digests (all, or only for --task)"
    )
    ...
    image_lock.add_argument(
        ...
        action="append",
        default=[],
        help="resolve only this task's image and merge into --output; repeatable",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "sync-upstream":
            ...
            lock = write_image_lock(args.upstream, args.output, tasks=args.tasks)
            result = {
                "output": str(args.output),
                "upstream_commit": commit,
                "scope": "tasks" if args.tasks else "complete",
                "locked_images": len(lock["images"]),
